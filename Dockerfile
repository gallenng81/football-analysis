...
from market_tracker import market_movements, steam_moves
...
with tab_odds:
    if not is_premium(user_tier):
        ...
        st.subheader("Market-wide odds movement")
        st.caption("Tracks odds across every match logged over time - either via the automated "
                   "GitHub Actions tracker (see fetch_market_odds.py) or manual snapshots below. "
                   "Purely informational, not a betting recommendation.")

        movements = market_movements()
        if movements.empty:
            st.info("No market history yet. Either wait for the automated tracker to log a few "
                    "snapshots (see .github/workflows/track_odds.yml), or log manual snapshots "
                    "for individual matches below.")
        else:
            st.markdown("**Biggest movers**")
            st.dataframe(movements, use_container_width=True)

            st.markdown("**Steam moves** (odds shortened notably since first tracked)")
            steam = steam_moves()
            if steam.empty:
                st.caption("No notable steam moves detected yet.")
            else:
                st.dataframe(steam, use_container_width=True)

        st.divider()
        st.subheader("Single match lookup")
